...
import { useCountries } from "@payconstruct/fe-utils/dist/Hooks/useCountries";

const { currencyWithNetworkCurrencyName } = Helpers;

const ManualPayoutsApproval: React.FC = () => {
  ...
  const filteredQueue = useMemo(() => {
    return approvalQueue
      .filter((queueItem: any) => {
        return queueItem?.type === "crypto_payout";
      })
      .sort(
        (a: any, b: any) =>
      ...
      <TransactionList
        ...
        accordionData={filteredQueue.map((payment: Transaction) => {
          ...
          return {
            ...
            headerSubtitle:
              (payment?.withdrawalAddress && `${payment?.withdrawalAddress}`) ||
              "",
            headerRight: (
              <React.Fragment>
                <span>
                ...
                <div style={{ marginTop: "-15px" }}>
                  <TransactionCard
                    ...
                    data={{
                      ...
                      "Payout amount": `${stringDataValidation(
                        selectedTransaction?.targetAmount
                      )}                   
                      ${stringDataValidation(
                        currencyWithNetworkCurrencyName(
                          selectedTransaction?.targetCurrency,
                          selectedTransaction?.mainCurrency,
                          currencies
                        ) || ""
                      )}
                      `,
                      "Target Currency": `${stringDataValidation(
                        selectedTransaction?.targetAmount
                      )}                   
                      ${stringDataValidation(
                        currencyWithNetworkCurrencyName(
                      ...
                      Account: `${stringDataValidation(
                        selectedTransaction?.merchantId ||
                          selectedTransaction?.targetMerchantId
                      )}                   
                      `,
                      Destination: `${stringDataValidation(
                        selectedTransaction?.withdrawalAddress
                      )}                   
                      `,
                      "User Full Name": `${stringDataValidation(
                        `${stringDataValidation(
                      ...
                      `,
                      "Country of Residence": `${stringDataValidation(
                        `${
                          countries?.find(
                            (country) =>
                              country?.alpha2Code ===
                              selectedTransaction?.countryOfResidence
                          )?.name || ""
                        }`
                      )}                   
                      `
